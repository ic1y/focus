...
const isIOS = iOS();
if (isIOS === false) document.body.classList.add("notiOS"); // thankfully
...
const showToDo = async (toDo, done) => {
	...
	deleteBtn.ariaLabel = `Delete to-do '${toDo}'`;
	deleteBtn.innerHTML = (isIOS) ? "Delete" :
	...
	deleteBtn.addEventListener("click", async () => {
		if (confirm(`Please confirm: delete '${toDoName.innerText}'?`) !== true)
			return; // require user confirmation for toDo deletion
		const rawResponse = await fetch("/addToDo", {
			method: "POST",
			headers: {
				Accept: "application/json",
...
document.getElementById("openMusic").addEventListener(
	"click",
	() => {
